	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		if(args.length>0){
			...
			if(teamOne.getPlayers().size()>teamTwo.getPlayers().size()){
				ScoreboardUtils.joinTeam((Player)sender, ChatColor.stripColor(teamTwo.getDisplayName()));
			} else {
				ScoreboardUtils.joinTeam((Player)sender, ChatColor.stripColor(teamOne.getDisplayName()));
			}
		}
		return true;
	}
